...
namespace ivLab3D 
{
    ...
    T* MakeNewGradient(const T* OldGradient, const int Width, const int Height, const int Depth)
        // Making a new gradient image surrounded with sentinels.
    {
        ...
        try
        {
            NewGradient = new T[(Width+2)*(Height+2)*(Depth+2)];
        }
        catch (std::bad_alloc &)
        {
            throw i3d::InternalException("ivLab3D::MakeNewGradient => cannot allocate memory");
        }
        ...
        for(x=0; x<(Width+2)*(Height+2); x++) {
            new_scan[x] = SentinelValue;
        }
        new_scan += (Width+2)*(Height+2);

        for (int z = 1; z<=Depth; z++)
        {
            for (x=0; x<(Width+2); x++)
            {
                *new_scan = SentinelValue;
                new_scan++;
            }
